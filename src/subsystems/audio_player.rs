...
impl AudioPlayer {
    pub fn new(cache_path : PathBuf, config: mpsc::Sender<ConfigCommand>, rx: mpsc::Receiver<PlayerCommand>) -> Self {
        Self {
    ...
    async fn process(&mut self) -> Result<()> {
        ...
        thread::spawn(move || {
            ...
            volume_change_wav
                .load_mem(ASSETS_DIR.get_file(PathBuf::from("volume-change.mp3")).unwrap().contents())
                .unwrap();
            ...
            let mut handle_command = |soloud: &mut Soloud, command| {
                ...
                match command {
                    PlayAsset { path, done } => {
                        ...
                        return Some(PlayState { handle: soloud.play(&play_wav), done });
                    },
                    PlayFile { path, done } => {
        ...
        self.config.send(ConfigCommand::GetVolume { responder: config_tx }).await?;
        let mut volume_config = config_rx.await?;

        soloud_tx.send(SoloudCommand::SetVolume { volume: volume_config.current }).await?;
        ...
        while let Some(play_command) = self.rx.recv().await {
            ...
            match play_command {
                PlayBoop { done } => {
                    let path = self.boop_paths.choose(&mut rand::thread_rng())
                        .ok_or(anyhow!("No boop files available"))?.clone();
                    soloud_tx.send(SoloudCommand::PlayAsset { path, done }).await?;
                },
                PlayConfirm { done } => {
                    let path = self.confirm_paths.choose(&mut rand::thread_rng())
                        .ok_or(anyhow!("No confirm files available"))?.clone();
                    soloud_tx.send(SoloudCommand::PlayAsset { path, done }).await?;
                },
                PlayAsset { path, done } => {
                    soloud_tx.send(SoloudCommand::PlayAsset { path, done }).await?;
                ...
                PlayCached { path, done } => {
                    soloud_tx.send(SoloudCommand::PlayFile { path: self.cache_path.join(path), done }).await?;
                },
                SetVolume { volume } => {
                    ...
                    let (config_tx, config_rx) = oneshot::channel();
                    self.config.send(ConfigCommand::SetVolume {
                ...
                GetVolume { responder } => {
                    soloud_tx.send(SoloudCommand::GetVolume { responder }).await?;
                },
                SetMaxVolume { volume } => {
    ...
    fn collect_paths(dir_name: &str) -> Result<Vec<PathBuf>> {
        ...
        for file in ASSETS_DIR.get_dir(dir_name).ok_or(anyhow!("Directory missing: {}", dir_name))?.files() {
            let path = file.path();
